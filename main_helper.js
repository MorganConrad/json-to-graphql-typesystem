...
function loadData(opts, args) {
  if (opts.uri) return mongoUtils.getFirstDocuments(opts.uri, args);
  else if (opts.url) return urlUtils.getJSOBFromURL(opts.url, opts);
  else {
    return Promise.resolve(args).then((filenames) =>
      filenames.map((filename) => fileutils.loadFile(filename, opts))
    );
  }
}
